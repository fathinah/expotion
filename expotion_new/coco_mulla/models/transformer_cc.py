...
class StreamingMultiheadAttention(StreamingModule):
    ...
    def _get_mask(self, current_steps: int, device: torch.device, dtype: torch.dtype):
        ...
        if self.memory_efficient:
            ...
            # print("streaming_state",self._streaming_state)
            if current_steps == 1:
    ...
    def forward(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor,
        ...
        if self.causal:
            ...
            # print(self._streaming_state)
            attn_mask = self._get_mask(query.shape[1], query.device, query.dtype)
            # attn_mask = None
        # assert attn_mask is None

        tag = "cross" if self.cross_attention else "self"
        if self.custom:
            ...
            if self.memory_efficient:
                ...
                if _efficient_attention_backend == 'torch':
                    ...
                    # projected = emb_fn.get_lora(tag, query) + projected
                    if emb_fn is not None and tag == "self":
                        adapter, gate = emb_fn.get_adaptor(tag)
                        if adapter is not None:
                            prefix_q, prefix_k, prefix_v = adapter
                            dt_q = emb_fn.crop(tag, prefix_q)
                            dt_q = dt_q + q
                            dt_x = torch.nn.functional.scaled_dot_product_attention(
                                dt_q, prefix_k, prefix_v, is_causal=False, dropout_p=p)
                            x = emb_fn.modify(x, dt_x, gate)
                else:
...
class StreamingTransformer(StreamingModule):
    ...
    def forward(self, emb_fn, x: torch.Tensor, *args, **kwargs):
        ...
        for i, layer in enumerate(self.layers):
            emb_fn.set_index(i)
            x = self._apply_layer(layer, emb_fn, x, *args, **kwargs)
