...
class Dataset(BaseDataset):
    ...
    def load_data(self, idx):
        tmp = 0
        while True:
            vid_id = idx if tmp==0 else random.sample(self.data,1)[0]
            mix_id = os.path.join(os.path.dirname(vid_id).replace("video","rvq"),os.path.basename(vid_id)).replace(".mp4",".npy").strip()
            feature_id = os.path.join(os.path.dirname(vid_id).replace("video","face"),os.path.basename(vid_id)).replace(".mp4",".pth").strip()
            if os.path.exists(mix_id) and os.path.exists(feature_id):
                break
            else:
                print(" idvid issssss:", vid_id)
                print(f"err in reading file {mix_id},{feature_id}")
                tmp=1

        mix = np.load(mix_id)
        face = torch.load(feature_id)
        face = face['face_p'].cpu().detach().numpy()
        result = {
            "mix": mix,
            "face": face
        }
